...
export class AdsController {
  ...
  @ApiOkResponse({
    ...
    type: PaginatedResponseDto<AdDto>,
  })
  @ApiBadRequestResponse({
    description: 'Validation error',
    type: BadRequestDto,
  ...
  async findAllAds(
    @Query() filterData: FilterAdDto,
    @UserInfo() userData: IJwtPayload,
  ): Promise<PaginatedResponseDto<AdDto>> {
    const [ads, count] = await this.service.findAllAds(
      filterData,
      userData?.profileId,
    );
  ...
  async getRecommendedAds(
    @Param('id', ParseIntPipe) id: number,
    @Query() recommendedAdsData: RecommendedAdsDto,
  ): Promise<AdDto[]> {
    const ads: Ad[] = await this.service.getRecommendedAds(
      id,
      recommendedAdsData,
    );
