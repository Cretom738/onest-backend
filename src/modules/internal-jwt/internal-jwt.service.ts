...
export class InternalJwtService {
    ...
    async verifyToken(token: string, type: EJwtTokenTypes): Promise<{ isTokenValid: boolean, payload: IJwtPayload }> {
        ...
        return {
            isTokenValid: !await this.isTokenBlacklisted(payload.accessTokenId, type),
            payload
        }
    }

    private async isTokenBlacklisted(tokenId: number, type: EJwtTokenTypes): Promise<boolean> {
        
        return !!await this.redis.exists(getBlacklistedTokenKey(tokenId, type));
    }
}
